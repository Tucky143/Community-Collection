...
public class CommunityCollectionModVariables {
	...
	public static void init(FMLCommonSetupEvent event) {
		CommunityCollectionMod.addNetworkMessage(SavedDataSyncMessage.TYPE, SavedDataSyncMessage.STREAM_CODEC, SavedDataSyncMessage::handleData);
	...
	public static void clonePlayer(PlayerEvent.Clone event) {
		...
		clone.wind_sound_timer = original.wind_sound_timer;
		if (!event.isWasDeath()) {
		}
	...
	public static class WorldVariables extends SavedData {
		public static final SavedDataType<WorldVariables> TYPE = new SavedDataType<>("community_collection_worldvars", ctx -> new WorldVariables(), ctx -> CompoundTag.CODEC.xmap(tag -> {
	...
	public static class MapVariables extends SavedData {
		public static final SavedDataType<MapVariables> TYPE = new SavedDataType<>("community_collection_mapvars", ctx -> new MapVariables(), ctx -> CompoundTag.CODEC.xmap(tag -> {
			MapVariables instance = new MapVariables();
			instance.read(tag, ctx.levelOrThrow().registryAccess());
			return instance;
		}, instance -> instance.save(new CompoundTag(), ctx.levelOrThrow().registryAccess())));
		boolean _syncDirty = false;
		public boolean FancyWeather = false;
		public boolean Thundering = false;
		public double SyncTimer = 0;

		public void read(CompoundTag nbt, HolderLookup.Provider lookupProvider) {
			FancyWeather = nbt.getBooleanOr("FancyWeather", false);
			Thundering = nbt.getBooleanOr("Thundering", false);
			SyncTimer = nbt.getDoubleOr("SyncTimer", 0);
		}

		public CompoundTag save(CompoundTag nbt, HolderLookup.Provider lookupProvider) {
			nbt.putBoolean("FancyWeather", FancyWeather);
			nbt.putBoolean("Thundering", Thundering);
			nbt.putDouble("SyncTimer", SyncTimer);
			return nbt;
		}
	...
	public record SavedDataSyncMessage(int dataType, SavedData data) implements CustomPacketPayload {
		public static final Type<SavedDataSyncMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(CommunityCollectionMod.MODID, "saved_data_sync"));
