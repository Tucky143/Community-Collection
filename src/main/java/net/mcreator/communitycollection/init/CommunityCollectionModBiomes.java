...
@EventBusSubscriber
public class CommunityCollectionModBiomes {
	@SubscribeEvent
		...
		for (LevelStem levelStem : levelStemTypeRegistry.stream().toList()) {
			...
			if (dimensionType.is(BuiltinDimensionTypes.OVERWORLD)) {
				...
				if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
					...
					addParameterPoint(parameters, new Pair<>(new Climate.ParameterPoint(Climate.Parameter.span(-1f, -0.5f), Climate.Parameter.span(-0.1f, 0.1f), Climate.Parameter.span(0.03f, 0.3f), Climate.Parameter.span(-0.5f, 0.5f),
							Climate.Parameter.span(0.4f, 1.1f), Climate.Parameter.span(-1f, 1f), 0), biomeRegistry.getOrThrow(ResourceKey.create(Registries.BIOME, ResourceLocation.fromNamespaceAndPath("community_collection", "ice_cave")))));
					chunkGenerator.biomeSource = MultiNoiseBiomeSource.createFromList(new Climate.ParameterList<>(parameters));
					chunkGenerator.featuresPerStep = Suppliers
							.memoize(() -> FeatureSorter.buildFeaturesPerStep(List.copyOf(chunkGenerator.biomeSource.possibleBiomes()), biome -> chunkGenerator.generationSettingsGetter.apply(biome).features(), true));
				}
				if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
					((CommunityCollectionModNoiseGeneratorSettings) (Object) noiseGenerator.settings.value()).setcommunity_collectionDimensionTypeReference(dimensionType);
				}
			}
		}
	}
	...
	private static SurfaceRules.RuleSource injectOverworldSurfaceRules(SurfaceRules.RuleSource currentRuleSource) {
		...
		customSurfaceRules.add(anySurfaceRule(ResourceKey.create(Registries.BIOME, ResourceLocation.fromNamespaceAndPath("community_collection", "ice_cave")), CommunityCollectionModBlocks.PERMAFROST.get().defaultBlockState(),
				Blocks.CALCITE.defaultBlockState(), Blocks.PACKED_ICE.defaultBlockState()));
	...
	public interface CommunityCollectionModNoiseGeneratorSettings {
		void setcommunity_collectionDimensionTypeReference(Holder<DimensionType> dimensionType);
	}
}